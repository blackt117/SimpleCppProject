...
void StudentRecords::get_course_names_grades(int sid)
{
	for (int i = 0; i < grades.size(); i++)
	{
		if (grades[i].get_student_id() == sid)
		{
			...
			{
				if (grades[i].get_course_id() == courses[q].get_id())
				{
					std::cout << courses[q].get_name() << "   ";
				}
			}
			std::cout << grades[i].get_grade()<< "\n";
		}
		/*
		for (int q = 0; q < grades.size(); q++)
		{
			if (courses[i].get)
		}
		*/
...
void StudentRecords::report_card(int sid)
{
	...
	std::cout << "Their grades for their respective classes are:" << "\n";
	get_course_names_grades(sid);
	std::cout << "Their GPA is: " << GPA;
}
